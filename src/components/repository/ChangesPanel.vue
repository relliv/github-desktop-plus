  ...
  <div ref="containerRef" class="flex-1 flex flex-col overflow-hidden">
    <div v-if="hasChanges" class="flex-1 flex flex-col overflow-hidden">
      ...
      <div
        ...
        :class="unstagedFiles.length > 0 ? 'max-h-[50%]' : 'flex-1'"
      >
        <div
          ref="stagedHeaderRef"
        ...
        <div v-else class="flex-1 space-y-0.5 px-4 overflow-y-auto">
          <FileItem
...
const calculateScrollerHeights = () => {
  ...
  const hasBothSections =
    stagedFiles.value.length > 0 && unstagedFiles.value.length > 0;

  if (hasBothSections) {
    // Split available height between both sections
    const halfHeight = availableHeight / 2;
    stagedScrollerHeight.value = Math.max(100, halfHeight - stagedHeaderHeight);
    unstagedScrollerHeight.value = Math.max(
      100,
      halfHeight - unstagedHeaderHeight
    );
  } else if (stagedFiles.value.length > 0) {
    stagedScrollerHeight.value = Math.max(
      100,
      availableHeight - stagedHeaderHeight
    );
  } else if (unstagedFiles.value.length > 0) {
    unstagedScrollerHeight.value = Math.max(
      100,
      availableHeight - unstagedHeaderHeight
    );
  }
};
