<template>
  <div
    class="flex flex-row gap-2 z-50"
    :class="[process.platform === 'darwin' ? 'pr-2' : 'pr-0']"
  >
    <!-- Window control buttons -->
    <button
      @click="minimizeWindow"
      class="size-8 flex items-center justify-center hover:bg-accent transition-colors rounded-md"
    ...
    <button
      @click="closeWindow"
      class="size-8 flex items-center justify-center hover:bg-destructive hover:text-destructive-foreground transition-colors rounded-md"
      :class="[process.platform === 'darwin' ? 'rounded' : '']"
    >
